...
fmt7:
  .asciz "\nThis is matrixS:\n"
fmt8:
  .asciz "\nThis is the inverse of matrixS:\n"
fmt9:
  .asciz "\nProof: matrixS x inverse =\n"
fmt10:
...
main:
  ...
  mov rdi, offset fmt1
  call printf
  mov rsi, offset matrixA
  call printm4x4
  # print matrixB
  mov rdi, offset fmt2
  call printf
  mov rsi, offset matrixB
  call printm4x4
  # compute the product matrixA x matrixB
  mov rdi, offset matrixA
  mov rsi, offset matrixB
  mov rdx, offset product
  call multi4x4
  # print the product
  mov rdi, offset fmt3
  call printf
  mov rsi, offset product
  call printm4x4
  # print matrixC
  mov rdi, offset fmt4
  call printf
  mov rsi, offset matrixC
  call printm4x4
  # compute the inverse of matrixC
  mov rdi, offset matrixC
  mov rsi, offset inverse
  call inverse4x4
  cmp rax, 1
  je singular
  # print the inverse
  ...
  mov rdi, offset fmt7
  call printf
  mov rsi, offset matrixS
  call printm4x4
  # compute the inverse of matrixS
  mov rdi, offset matrixS
  mov rsi, offset inverse
  call inverse4x4
  cmp rax, 1
  je singular
...
.four:
  ...
  .bss
  .align 32
  .lcomm .matrix1, 16 * 8 # intermediate matrix
  .lcomm .matrix2, 16 * 8 # intermediate matrix
  .lcomm .matrix3, 16 * 8 # intermediate matrix
  .lcomm .matrix4, 16 * 8 # intermediate matrix
  .lcomm .matrixI, 16 * 8
  .lcomm .mxcsr,   4      # used for checking zero division
...
inverse4x4:
  ...
  mov rsi, rdi
  mov rdx, offset .matrix2
  push rdi
  call multi4x4
  pop rdi
  # compute the intermediate matrix3
  mov rsi, offset .matrix2
  mov rdx, offset .matrix3
  push rdi
  call multi4x4
  pop rdi
  # compute the intermediate matrix4
  mov rsi, offset .matrix3
  mov rdx, offset .matrix4
  push rdi
  call multi4x4
  pop rdi
  # compute the traces
  ...
  mov rdi, offset .matrix2
  mov rsi, [.size]
  call vtrace
  movsd xmm9, xmm0  # trace 2 in xmm9
  # compute trace3
  mov rdi, offset .matrix3
  mov rsi, [.size]
  call vtrace
  movsd xmm10, xmm0 # trace 3 in xmm10
  # compute trace4
  mov rdi, offset .matrix4
  mov rsi, [.size]
  call vtrace
...
vtrace:
  ...
  vmovapd ymm2, [rdi+64]
  vmovapd ymm3, [rdi+96]
  vblendpd ymm0, ymm0, ymm1, 0b0010
  vblendpd ymm0, ymm0, ymm2, 0b0100
  vblendpd ymm0, ymm0, ymm3, 0b1000
  vhaddpd ymm0, ymm0, ymm0
  vpermpd ymm0, ymm0, 0b00100111
  haddpd xmm0, xmm0
  leave
  ret
